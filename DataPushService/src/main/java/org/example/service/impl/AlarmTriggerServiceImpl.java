...
public class AlarmTriggerServiceImpl implements AlarmTriggerService {
    private static final Logger logger = LoggerFactory.getLogger(AlarmTriggerServiceImpl.class);
    @Autowired
...
//    private AlarmMethodsServiceImpl alarmMethodsService;
    ...
    public void triggerAlarmAsync(String redisKey, AlarmConfigModel model, String message) {
        ...
        if (ListUtil.isNull(query)){
            logger.error(String.format("查无此报警设备 barCode:%s dataTypeId:%s address:%s",model.getBarCode(),model.getDeviceTypeId(),model.getDeviceAddress()));
        }
        ...
        if ("0".equals(lastValue)){
            ...
            //错误值不在定义范围
            logger.info(String.format("0值    箱子:%s deviceId:%d time:%s 报警字段%s 最后值:%s 错误值:%s 消息:%s",deviceName,deviceId,model.getTime(),model.getAttributeNameCn(),model.getLastValue(),errorInfo,message));
            return;
        }
        if (errorInfo.equals("正常")){
            //错误值不在定义范围
            logger.info(String.format("正常状态? 异常  箱子:%s deviceId:%d time:%s 报警字段%s 最后值:%s 错误值:%s 消息:%s",deviceName,deviceId,model.getTime(),model.getAttributeNameCn(),model.getLastValue(),errorInfo,message));
            return;
        }
...
//        List<String> phoneList = deviceMonitorRecordV2Repository.queryPhoneNumber(new ArrayList<>(homeInfoModel.getBdAreaLevelIdsSet()));
        ...
        if (ListUtil.isNull(ids)){
            //logger.error(String.format("查无此设备报警记录 barCode:%s dviceTypeId:%s address:%s deviceId:%d attributeId:%d",model.getBarCode(),model.getDeviceTypeId(),model.getDeviceAddress(),deviceId,attributeId));
            return;
        }
